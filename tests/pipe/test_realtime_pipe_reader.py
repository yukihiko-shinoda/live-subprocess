...
# Reason: ffmpeg-python's issue
import ffmpeg  # type: ignore[import-untyped]
import pytest

from livesubprocess.pipe.realtime_pipe_reader import FFmpegRealtimePipeReader
...
class TestStringRealtimePipeReader:
    ...
    def test_wait(self) -> None:
        """Test wait() returns (stdout, returncode) when process exits normally."""
        command = [sys.executable, "-c", "print('hello')"]
        # Reason: This only executes test code.
        with Popen(command, stdout=PIPE, stderr=PIPE) as popen:  # noqa: S603  # nosec
            reader = StringRealtimePipeReader(popen)
            stdout, returncode = asyncio.run(reader.wait())
        assert "hello" in stdout
        assert returncode == 0

    def test_wait_returncode_none(self) -> None:
        """Test wait() raises RuntimeError when popen.returncode is None after exit."""
        popen_mock = MagicMock()
        popen_mock.stdout = io.BytesIO(b"")
        popen_mock.stderr = io.BytesIO(b"")
        popen_mock.returncode = None
        reader = StringRealtimePipeReader(popen_mock)
        with pytest.raises(RuntimeError, match="Process finished but returncode is None"):
            asyncio.run(reader.wait())
